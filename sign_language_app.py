...
Yêu cầu: Python 3.8+
Cài đặt: pip install opencv-python mediapipe tensorflow numpy pillow
Chạy: python sign_language_app.py
"""
...
class SignLanguageApp:
    def __init__(self, root):
        self.root = root
        self.root.title("🤖 Học Ngôn Ngữ Ký Hiệu")
    ...
    def create_widgets(self):
        ...
        tk.Label(header, text="🤖 Hệ Thống Học Ngôn Ngữ Ký Hiệu", 
                font=('Arial', 20, 'bold'), bg='#0a0e1a', fg='white').pack()
        tk.Label(header, text="AI tự học từ bạn! Thu thập → Huấn luyện → Nhận diện",
    ...
    def create_collect_tab(self):
        ...
        self.log_collect = scrolledtext.ScrolledText(tab, height=10,
                                                     bg='#0f172a', fg='#9ca3af',
                                                     font=('Courier', 9),
                                                     relief=tk.FLAT)
    ...
    def create_recognize_tab(self):
        ...
        self.log_recognize = scrolledtext.ScrolledText(tab, height=10,
                                                       bg='#0f172a', fg='#9ca3af',
                                                       font=('Courier', 9),
                                                       relief=tk.FLAT)
            ...
            if ret:
                ...
                if results.multi_hand_landmarks:
                    for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                        # Màu khác nhau cho mỗi tay
                        color = (59, 130, 246) if idx == 0 else (16, 185, 129)
                    ...
                    elif self.notebook.index('current') == 2 and self.model:
    ...
    def extract_features(self, multi_hand_landmarks):
        features = []
        for i in range(2):
            if i < len(multi_hand_landmarks):
                for lm in multi_hand_landmarks[i].landmark:
    ...
    def _train_model_thread(self):
        try:
            ...
            self.model = keras.Sequential([
                keras.layers.Dense(64, activation='relu', input_shape=(126,)),
